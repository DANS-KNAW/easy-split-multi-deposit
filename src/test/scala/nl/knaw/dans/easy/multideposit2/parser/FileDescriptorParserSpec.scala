...
import nl.knaw.dans.easy.multideposit2.model.{ FileAccessRights, FileDescriptor }
...
trait FileDescriptorTestObjects {
  ...
  lazy val fileDescriptors = Map(
    mdDir.resolve("ruimtereis01/reisverslag/centaur.mpg") ->
      FileDescriptor(
        title = Option("video about the centaur meteorite"),
        accessibility = Option(FileAccessRights.RESTRICTED_GROUP)
      ),
    mdDir.resolve("ruimtereis01/path/to/a/random/video/hubble.mpg") ->
      FileDescriptor(
        title = Option.empty,
        accessibility = Option(FileAccessRights.RESTRICTED_GROUP)
      )
...
class FileDescriptorParserSpec extends TestSupportFixture with FileDescriptorTestObjects with BeforeAndAfterEach {
  ...
  private val parser = new FileDescriptorParser with ParserUtils with InputPathExplorer {
    def multiDepositDir: Path = mdDir
  }
  ...
  it should "fail if the path does not exist" in {
    ...
    fileDescriptor("ruimtereis01")(2)(row).value should matchPattern {
      case Failure(ParseException(2, "FILE_PATH 'path/that/does/not/exist.txt' does not exist", _)) =>
    }
  }
  ...
  it should "fail if an invalid FILE_ACCESSIBILITY is given and the path does not exist" in {
    ...
    fileDescriptor("ruimtereis01")(2)(row).value should matchPattern {
      ...
      ParseException(2, "FILE_PATH 'path/that/does/not/exist.txt' does not exist", _) ::
